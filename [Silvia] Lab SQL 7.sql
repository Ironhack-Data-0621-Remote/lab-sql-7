...
select * from actor 
where exists (select last_name, count(last_name) as total_repeat 
from actor 
group by last_name 
having total_repeat= 1);
...
select last_name, count(last_name) as total_repeats 
from actor 
group by last_name
having total_repeats > 1
order by last_name;
