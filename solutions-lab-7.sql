...
SELECT COUNT(actor_id) AS count_name, last_name
FROM actor
GROUP BY last_name
HAVING count_name = 1;
...
SELECT COUNT(rental_id) AS nbr_rentals, staff_id
FROM rental
GROUP BY staff_id;

-- 4. Using the film table, find out how many films were released each year.
SELECT COUNT(film_id) AS nbr_films, release_year
FROM film
GROUP BY release_year;

-- 5. Using the film table, find out for each rating how many films were there.
SELECT COUNT(film_id) AS nbr_films, rating
FROM film
GROUP BY rating;
